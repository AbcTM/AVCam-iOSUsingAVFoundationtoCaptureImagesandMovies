...
//        NotificationCenter.default.addObserver(self, selector: #selector(orientationDidChange(_:)), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
        ...
        if motionManager.isDeviceMotionAvailable {
            motionManager.startAccelerometerUpdates(to: queue!, withHandler: { [unowned self](accelerometerData, error) in

                guard let acceleration = accelerometerData?.acceleration else {
                    return
                }

                var orientationNew: UIDeviceOrientation? = nil
                
                
                if (acceleration.x >= 0.75) {
                    orientationNew = UIDeviceOrientation.landscapeRight
                }
                else if (acceleration.x <= -0.75) {//home button right
                     orientationNew = UIDeviceOrientation.landscapeLeft
                }
                else if (acceleration.y <= -0.75) {
                     orientationNew = UIDeviceOrientation.portrait
                }
                else if (acceleration.y >= 0.75) {
                     orientationNew = UIDeviceOrientation.portraitUpsideDown
                }
                else {
                    return
                }

                if let orientationNew = orientationNew, self.orientation != orientationNew {
...
//        print("设备方向发生变化:\(orientation.rawValue)")
//    }
